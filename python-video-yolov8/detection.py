...
From https://github.com/tinygrad/tinygrad/blob/master/examples/yolov8.py
"""
...
def draw_black_bounding_boxes_and_save(orig_img_path, output_img_path, predictions, class_labels):
  color_dict = {label: tuple((((i+1) * 50) % 256, ((i+1) * 100) % 256, ((i+1) * 150) % 256)) for i, label in enumerate(class_labels)}
  font = cv2.FONT_HERSHEY_SIMPLEX
  ...
  height, width, _ = orig_img.shape
  out_img = np.zeros_like(orig_img)
  box_thickness = int((height + width) / 400)
  font_scale = (height + width) / 2500
  object_count = defaultdict(int)
  ...
  for pred in predictions:
    ...
    out_img[y1:y2, x1:x2] = orig_img[y1:y2, x1:x2]
  ...
  for pred in predictions:
    ...
    color = color_dict[class_labels[class_id]]
    cv2.rectangle(out_img, (x1, y1), (x2, y2), color, box_thickness)
    label = f"{class_labels[class_id]} {conf:.2f}"
    text_size, _ = cv2.getTextSize(label, font, font_scale, 1)
    label_y, bg_y = (y1 - 4, y1 - text_size[1] - 4) if y1 - text_size[1] - 4 > 0 else (y1 + text_size[1], y1)
    cv2.rectangle(out_img, (x1, bg_y), (x1 + text_size[0], bg_y + text_size[1]), color, -1)
    font_color = (0, 0, 0) if is_bright_color(color) else (255, 255, 255)
    cv2.putText(out_img, label, (x1, label_y), font, font_scale, font_color, 1, cv2.LINE_AA)
    object_count[class_labels[class_id]] += 1
...
def process_images(process_args):
  (list_images, yolo_variant) = process_args
  ...
  # run for each images
  for (img_path, out_path, black, save_json) in list_images:
    ...
    if black:
      draw_black_bounding_boxes_and_save(orig_img_path=image_location, output_img_path=out_path, predictions=predictions, class_labels=class_labels)
    else:
...
def main():
  ...
  yolo_variant = "n"
  print(f'Using YOLO version {yolo_variant}')
  ...
  files = [
        (f"./{one_file}", output_folder_path / one_file, black, save_json) for one_file in files
  ]

  process = 5
  groups = [[] for _ in range(process)]
  for i, f in enumerate(files):
      groups[i % 5].append(f)
  groups = [(one_group, yolo_variant) for one_group in groups]

  multi_process(process_images, groups, process)
